...
3. If any answer is blank, do not fabricate or partially finalize scores. Identify the unanswered question numbers and leave progress unchanged. In both normal (understanding/application) and `term-recall` sessions, an explicit response such as `わかりません` is an answered question, not a blank: score it `0 / 100`, state that no answer could be recalled, and include the compact model explanation and next-review focus.
