...
- 科目A免除に合わせ、日々の出題計画ではTrack `A`を候補から除外する。Track `A/B`は科目B・午後問題の前提にもなるため候補に残す。
- 暗記語句Sessionの新規作成分では、候補に残るTrack `B`と`A/B`をどちらもSession Track `B`として扱う。カタログ自体のTrackと語句表記は変更しない。
