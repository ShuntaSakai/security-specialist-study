...
問題数は指定がなければ10問。指定時は1〜30問の範囲とする。科目A免除に合わせてカタログのTrack `A` は出題候補から除外する。新規作成分ではカタログのTrack `B` と、候補に残るTrack `A/B` をどちらもSession Track `B` として出題する。既存の旧Sessionに限り、以前のA/B 4:6配分でも採点できる。`進捗/語句別理解度.md` のTrackはカタログの値を維持する。
...
暗記語句Sessionではさらに、各問のPrimary Termsが1項目、全問がLevel 1、TrackがリテラルのAまたはBであることも検証する。新規作成分は全問Track Bで、既存の旧Sessionに限り以前のA/B 4:6配分も受け入れる。これにより、一つの回答で複数語句を評価したり、暗記問題を高難易度の証拠として誤反映したりすることを防ぐ。
